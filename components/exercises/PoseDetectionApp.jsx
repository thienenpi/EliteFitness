...
const PoseDetectionApp = memo(
  ({ practiceState, recordState, cameraState, item, onUpdateCounter }) => {
      ...
      const fetchData = async () => {
        ...
        try {
          const response = await axios.get(
            `http://172.0.1.94:3000/api/exercises/${item._id}`
          );
      ...
      try {
        ...
        if (prevAngles) {
          ...
          if (duration > 500) {
            ...
            velocities = util.calculateVelocity(
              prevAngles,
              currAngles,
              duration
            );
        ...
        >
          <TensorCamera
            ref={(e) => {
              ...
              // console.log("a")
              // cameraRef.current = e
              e.camera.takePictureAsync().then(success => success.uri)
               setCamera(e);

              // if (!e || !e.camera || !e.camera.recordAsync || !e.camera.stopRecording) return
              //  e.camera.recordAsync().then(success=> {
              //   const { uri } =  success;
              //   console.log(uri)
              // }).catch(e => {
              //   console.log('e', e)
         ...
         {recordURI &&   <Video
        // ref={e=> e}
        style={{ position: 'absolute', top: 200, left: 10, zIndex: 998, width: 600, height: 500}}
        source={{
          uri: recordURI,
        }}
        useNativeControls
